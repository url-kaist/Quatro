...
struct RotResidual {
  RotResidual(Eigen::Vector3d src, Eigen::Vector3d tgt) : src_(src), tgt_(tgt) {}

  template <typename T> bool operator()(const T* const angle_est, T* residual) const {
    residual[0] = sqrt(pow(tgt_(0) - src_(0) * cos(*angle_est) - src_(2) * sin(*angle_est), 2) +
                       pow(tgt_(1) - src_(1), 2) +
                       pow(tgt_(2) - src_(2) * cos(*angle_est) + src_(0) * sin(*angle_est), 2));
    return true;
  }
...
public:
  ...
  virtual void solveForRotation(const Eigen::Matrix<double, 3, Eigen::Dynamic>& src,
                                ...
                                Eigen::Matrix3d* rotation,
                                Eigen::Matrix<bool, 1, Eigen::Dynamic>* inliers) = 0;
  virtual void solveForRotation2D(const Eigen::Matrix<double, 2, Eigen::Dynamic>& src,
...
public:
  ...
  virtual void solveForTranslation(const Eigen::Matrix<double, 3, Eigen::Dynamic>& src,
                                   ...
                                   bool using_median_selection=false) = 0;
...
public:
  ...
  * @param using_median_selection consensus의 median을 고를지 말지 결정하는 파라미터
  *        false이면 weighted mean 값이 리턴됨
  */
  void solveForTranslation(const Eigen::Matrix<double, 3, Eigen::Dynamic>& src,
                          const Eigen::Matrix<double, 3, Eigen::Dynamic>& dst,
                          Eigen::Vector3d* translation,
                          Eigen::Matrix<bool, 1, Eigen::Dynamic>* inliers,
                          bool using_median_selection=false) override;
...
public:
  ...
  GNCRotationSolver(Params params) : params_(params) {
    params_.noise_bound = params.noise_bound;
  }
...
public:
  ...
  struct Params {
     ...
     * TEASER (TEASER++ 을 나타냄) 이나 Quatro 중 택 1
     */
    std::string reg_name = "TEASER"; // "TEASER" or "Quatro"
     ...
     *  "median" (Ours) 모드가 있음
     */
    std::string cote_mode = "weighted_mean";
  ...
  void getTargetNormals(Eigen::Matrix<double, 3, Eigen::Dynamic>& target_normals){
    tgt_normals.resize(3, target_normals.cols());
  ...
  Eigen::Vector3d solveForTranslation(const Eigen::Matrix<double, 3, Eigen::Dynamic>& v1,
                                      const Eigen::Matrix<double, 3, Eigen::Dynamic>& v2,
                                      const Eigen::Matrix<double, 3, Eigen::Dynamic>& normal);
  ...
  inline std::vector<int> getFinalInliers() { return final_inliers_; }
   ...
   * @return
   */
  // inline Eigen::Matrix<int, 2, Eigen::Dynamic>;
  ...
  void reset(const Params& params) {
    ...
    if (params_.estimate_scaling) {         // quatro_examples의 setparams 에서 false로 해줌 => ScaleInliersSelector를 가리키는 스마트 포인터  scale_solver_ -> (ScaleInliersSelector)
      setScaleEstimator(
    ...
    //    std::cout<<"\033[1;33m What? "<< params_.noise_bound<<"\033[0m"<<std::endl;
    ...
    case ROTATION_ESTIMATION_ALGORITHM::GNC_TLS: { // GNC-TLS method
      //      std::cout<<"\033[1;34mNot comming??\033[0m"<<std::endl;
