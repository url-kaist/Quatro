...
//  std::string SAVE_PATH = "/home/shapelim/kitti_output/weights";
//  std::string svdPath;
//  std::ofstream svdTxt;
//  svdPath = (boost::format("%s/TEASER_%04d_singular_values.txt") % SAVE_PATH % static_count).str();
//  // save singular values
//  svdTxt.open(svdPath, std::ios::app);
//  const auto& sgValues = svd.singularValues();
//  svdTxt<<sgValues(0,0)<<" "<<sgValues(0,1)<<" "<<sgValues(0, 2)<<"\n";
//  svdTxt.close();
 ...
 */
template <class T> inline std::vector<int> findNonzero(Eigen::Matrix<T, 1, Eigen::Dynamic> mask) {
